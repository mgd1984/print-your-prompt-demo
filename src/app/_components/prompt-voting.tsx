    ...
    <div className="w-full flex flex-col lg:flex-row gap-6">
        ...
        {isActive ? (
          <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-[#e5e7eb] h-full">
            <div className="p-5 border-b border-[#e5e7eb]">
              <h3 className="text-xl font-semibold text-[#1e3a8a]">Submit Your Prompt</h3>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="w-full bg-[#e6f0ff] rounded-full h-2.5 mb-5">
                <div 
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-md border border-[#e5e7eb] bg-white p-3 text-[#1e3a8a] placeholder:text-[#94a3b8] focus:outline-none focus:ring-2 focus:ring-[#4285f4] text-base"
                    value={username}
                ...
                <div className="relative">
                  <textarea
                    ...
                    className="w-full rounded-md border border-[#e5e7eb] bg-white p-4 text-[#1e3a8a] placeholder:text-[#94a3b8] focus:outline-none focus:ring-2 focus:ring-[#4285f4] min-h-[180px] text-base"
                    value={prompt}
                    ...
                    disabled={!isActive || isSubmitting}
                  />
                  <div className="absolute bottom-2 right-2 text-xs text-[#3b82f6]/60">
                    {prompt.length}/500
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-[#4285f4] hover:bg-[#3b7bf2] text-white font-medium rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-base"
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-[#e5e7eb] h-full">
            ...
            <div className="p-6 text-center">
              <h4 className="text-xl font-semibold text-[#1e3a8a] mb-4">Voting has ended!</h4>
              {topPrompt ? (
                <div>
                  <p className="mb-2 text-[#3b82f6]/80">The winning prompt is:</p>
                  {topPrompt.username && (
                    <p className="text-sm font-medium text-[#3b82f6] mb-2">
                      by <span className="inline-block bg-[#4285f4]/10 px-2 py-0.5 rounded-full">{topPrompt.username}</span>
                    </p>
                  )}
                  <p className="text-xl font-bold text-[#4285f4] mb-4 px-4">
                    "{topPrompt.text}"
                  </p>
              ...
              <div className="mt-8">
                <Link
                  href="/gallery"
                  className="text-[#4285f4] hover:text-[#1e3a8a] transition-colors flex items-center justify-center"
                >
      ...
      <div className="w-full lg:w-5/12">
        <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-[#e5e7eb] h-full flex flex-col">
          <div className="p-5 border-b border-[#e5e7eb]">
            <h3 className="text-xl font-semibold text-[#1e3a8a]">{isActive ? "Current Prompts" : "All Prompts"}</h3>
          </div>
          ...
          <div className="flex-1 overflow-hidden">
            {promptsQuery.isLoading ? (
              ...
              <div className="p-4 overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-[#e5e7eb] scrollbar-track-transparent">
                <ul className="space-y-2">
                  {sortedPrompts.map((prompt) => (
                    <li 
                      key={prompt.id} 
                      className={`flex flex-col p-4 rounded-md border border-[#e5e7eb] bg-[#f8fafc] ${isActive ? 'hover:border-[#4285f4]/40 hover:bg-[#f0f7ff] cursor-pointer transition-colors group' : ''}`}
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-[#1e3a8a] mr-2 text-sm">{prompt.text}</span>
                        <span className={`bg-[#4285f4]/10 text-[#4285f4] px-3 py-1 rounded-full text-xs font-medium transition-colors whitespace-nowrap ${isActive ? 'group-hover:bg-[#4285f4]/20' : ''}`}>
          ...
          <div className="p-4 border-t border-[#e5e7eb] bg-[#f8fafc]">
            <p className="text-xs text-center text-[#3b82f6]/80">
              {isActive ? "Click on a prompt to vote for it" : "These prompts were submitted during the voting period"}
            </p>
          </div>
        </div>
      </div>
